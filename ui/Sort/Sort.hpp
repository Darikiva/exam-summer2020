...
constexpr void heapify(It first, It last, std::vector<change::Change<It>> &changes, const Comp comparator) {
    if (first == last) {
        return;
    }
    ...
    while (begin != last) {
        ...
        while ((begin_pos != 0) && (comparator(*father, *begin))) {
            changes.emplace_back(change::Swap<It>{father, begin});
 ...
 */
template<typename It, typename Comp>
constexpr void restore_heap(It first, It last, std::vector<change::Change<It>> &changes, Comp comparator) {
    if (first == last) {
        return;
    }
    ...
    do {
        ...
        if (comparator(*current, *selected_child)) {
            move_next = true;
            changes.emplace_back(change::Swap<It>{current, selected_child});
...
constexpr void quickSort(FIter first, FIter last, std::vector<change::Change<FIter>> &changes, Comp comparator, std::forward_iterator_tag) {
    ...
    changes.emplace_back(change::Swap<FIter>{std::next(first, distance / 2), end});
    std::iter_swap(std::next(first, distance / 2), end);

    auto mid = iterPartition(first, end, [&](const auto &el) {
        if (comparator(*el, *end)) {
            changes.emplace_back(change::Swap<FIter>{el, end});
            return true;
        }
        return false;
    });
    std::iter_swap(mid, end);
    changes.emplace_back(change::SelectPivot<FIter>{mid});
...
constexpr void quickSort(RAIter first, RAIter last, std::vector<change::Change<RAIter>> &changes, Comp comparator, std::random_access_iterator_tag) {
    const size_t distance = std::distance(first, last);
    ...
    changes.emplace_back(change::Swap<RAIter>{std::next(first, distance / 2), end});
    std::iter_swap(std::next(first, distance / 2), end);

    auto mid = iterPartition(first, end, [&](const auto &el) {
        if (comparator(*el, *end)) {
            changes.emplace_back(change::Swap<RAIter>{el, end});
            return true;
        }
        return false;
    });
    std::iter_swap(mid, end);
    changes.emplace_back(change::SelectPivot<RAIter>{mid});
 ...
 */
template<typename It, typename Comp>
void selectionSort(It first, It last, std::vector<change::Change<It>> &changes, Comp comp, std::bidirectional_iterator_tag) {
        for (auto it = first; it != last; ++it) { 
            auto key = it;
            if (key != first) {
                while ((key != first) && (comp(*key, *std::prev(key)))) {
                    changes.emplace_back(change::Swap<It>{key, std::prev(key)});
 ...
 */
template<typename It, typename Comp>
void selectionSort(It first, It last, std::vector<change::Change<It>> &changes, Comp comp, std::forward_iterator_tag) {
        for (auto it = first; it != last; ++it) { 
            auto key = it;
            if (key != first) {
                auto prev = std::next(first, std::distance(first, key)-1);
                while ((key != first) && (comp(*key, *prev))) {
                    changes.emplace_back(change::Swap<It>{key, prev});
 ...
 */
template<typename It, typename Comp>
void advanced_merge(It first, It middle, It last, std::vector<change::Change<It>> &changes, Comp comp) {
    ...
    for (int fixed_index = 0; fixed_index < indexes.size(); ++fixed_index) {
        if (indexes[fixed_index] != fixed_index) {
            ...
            changes.emplace_back(change::Swap{std::next(first, indexes[pos]), std::next(first, indexes[fixed_index])});
        }
    }
}
...
struct Sort<type::Insertion>
{
    ...
    auto operator()(It first, It last) const -> std::vector<sort::change::Change<It>>
    {
...
struct Sort<type::Selection>
{
    ...
    template<typename It>
    auto operator()(It first, It last) const -> std::vector<change::Change<It>>
    {
        return (*this)(first, last, std::less{});
    }
};
        ...
        while (has_changed) {
            ...
            while (next_iterator != last) {
                if (comp(*next_iterator, *begin)) {
                    changes.emplace_back(change::Swap{next_iterator, begin});
...
struct Sort<type::Merge>
{
    ...
    auto operator()(It first, It last, Comp comp) const -> std::vector<change::Change<It>>
    {
        ...
        if (const auto distance = std::distance(first, last); distance > 1) {
            ...
            changes.emplace_back(change::SelectSubrange<It>{first, middle});
            auto left_changes = (*this)(first, middle, comp);
            changes.insert(
                changes.end(), 
                std::make_move_iterator(left_changes.begin()), 
                std::make_move_iterator(left_changes.end())
            );

            changes.emplace_back(change::SelectSubrange<It>{middle, last});
            auto right_changes = (*this)(middle, last, comp);
            changes.insert(
                changes.end(), 
                std::make_move_iterator(right_changes.begin()), 
                std::make_move_iterator(right_changes.end())
            );
                
            changes.emplace_back(change::MergeSubranges<It>{first, last});
...
struct Sort<type::Heap>
{
    ...
    auto operator()(It first, It last, Comp comp) const -> std::vector<change::Change<It>>
    {
        ...
        while (size >= 0) {
            ...
            changes.emplace_back(change::Swap{to_swap, first});
